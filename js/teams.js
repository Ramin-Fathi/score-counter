...
function startHold(button, scoreDisplay, callback) {
  ...
  // Reduced the time to 1.5s for demonstration, you can change it back to 2s if you want
...
/**
 * Show the reset countdown popup, do a 3-second countdown, then reset.
 */
function showResetCountdown() {
  // Create a circular popup
  const resetCircle = document.createElement('div');
  ...
  document.body.appendChild(resetCircle);

  // Animate heartbeat via CSS (defined in style)
  ...
  const interval = setInterval(() => {
    ...
    if (timeLeft <= 0) {
      ...
      document.body.removeChild(resetCircle);
      resetGame();
    } else {
      countdown.innerText = timeLeft;
    }
